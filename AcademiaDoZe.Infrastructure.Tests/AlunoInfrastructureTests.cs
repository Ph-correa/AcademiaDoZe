...
public class AlunoInfrastructureTests : TestBase
{
    ...
    internal static async Task<Aluno> CriarEInserirAlunoAsync(AlunoRepository alunoRepo, LogradouroRepository logradouroRepo)
    {
        ...
        var alunoResult = Aluno.Criar(
            id: 0,
            nome: "Pedro " + Guid.NewGuid().ToString("N")[..5],
            cpf: GerarCpf(),
            dataNascimento: new DateOnly(2000, 10, 20),
            telefone: GerarTelefone(),
            email: GerarEmail(),
            endereco: logradouro,
            numero: "100",
            complemento: "Henrique Correa",
            senha: "Mysql123!", // Corrigido: adicionado '!' para atender às regras do SENHA_FORMATO
            foto: foto
        );
    ...
    public async Task Aluno_ObterPorNome_Sucesso()
    {
        ...
        string parteDoNome = aluno.Nome.Substring(0, 8);
        var resultados = await _alunoRepo.ObterPorNome(parteDoNome);
    ...
    public async Task Aluno_Atualizar_Sucesso()
    {
        ...
        var aluno = await _alunoRepo.Adicionar(Aluno.Criar(
            0, "Aluno Original", GerarCpf(), new DateOnly(2000, 10, 20),
            GerarTelefone(), GerarEmail(), logradouro, "100", "Apto 101",
            "SenhaValida123!", foto).Value!);
