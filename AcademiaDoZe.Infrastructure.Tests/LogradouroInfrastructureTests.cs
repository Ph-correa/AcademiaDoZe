...
public class LogradouroInfrastructureTests : TestBase
{
    ...
    {
        var cep = GerarCep();
        // Atende aos requisitos da atividade: Rua = Nome, Bairro = Sobrenome, Cidade = SQLite
        var logradouroResult = Logradouro.Criar(0, cep, "Pedro Henrique", "Correa", "SQLite", "SC", "Brasil");
    ...
    {
        var cep = GerarCep();
        var logradouro = Logradouro.Criar(0, cep, "Pedro Henrique", "Correa", "SQLite", "SC", "Brasil").Value!;
        ...
        Assert.Equal("Pedro Henrique", inserido.Nome);
    ...
    public async Task Logradouro_ObterPorCidade_FiltragemCorreta()
    {
        ...
        var logradouro = Logradouro.Criar(0, cep, "Pedro Henrique", "Correa", cidadeUnica, "SC", "Brasil").Value!;
        await _repository.Adicionar(logradouro);

        // Removido o .ToLower() para corresponder exatamente à string gravada no SQLite
