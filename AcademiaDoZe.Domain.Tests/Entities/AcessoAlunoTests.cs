...
{
    // Garantindo uma Segunda-feira fixa para não depender do dia atual (DateTime.Today)
    private static readonly DateTime SegundaFeiraValida = new DateTime(2026, 1, 5); // 05/01/2026 = Segunda-feira
    ...
    // CPF válido com algoritmo correto
    private static Aluno GetValidAluno() => Aluno.Criar(
        1,
        "João",
        "52998224725",
        DateOnly.FromDateTime(SegundaFeiraValida.AddYears(-20)),
        "(11) 91234-5678",
        "user@example.com",
        GetValidLogradouro(),
        "123",
    ...
    public void Deve_Falhar_Criacao_Quando_AlunoEhNulo(bool alunoNull)
    {
        ...
        var result = AcessoAluno.Criar(1, aluno!, SegundaFeiraValida.AddHours(10));
    ...
    public void Deve_Falhar_Criacao_Quando_HorarioForaDoIntervalo(int hour, int minute)
    {
        ...
        var dataHora = SegundaFeiraValida.AddHours(hour).AddMinutes(minute);
    ...
    {
        var aluno = GetValidAluno();
        var dataHora = SegundaFeiraValida.AddHours(hour);
    ...
    {
        var aluno = GetValidAluno();
        var time = SegundaFeiraValida.AddHours(hour);
